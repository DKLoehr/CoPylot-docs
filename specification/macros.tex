...
\newcommand{\osCast}[8]{
  ...
  \relationRule{#1}{%
    ...
    %
    \osMC{Call} \;#8], \listConcat \omstack, \ostack, \oheap \osTransition%
    \osInStack{\omstack'}, \ostack, \oheap%
  }%
}

\newcommand{\osCastGraph}[8]{
  % #1: magic name, #2: first type, #3: second type list
  % #4: len(list), #5: MI to call, #6: other things to put before the micro-stack,
  % #7: bind/assign #8: arg number
  \relationRule{#1}{%
    ...
    \ifthenelse {\equal{#8}{2}} {%
      \gnode_2 = \gsLR{\olbl, \osInStack{\ovalue, \omem_1, \omem_2, \osMC{Call} \;2}, \gtime}%
    } {%
      \ifthenelse {\equal{#8}{1}} {%
        \gnode_2 = \gsLR{\olbl, \osInStack{\ovalue, \omem \osMC{Call} \;1}, \gtime} %
      } {}%
    }%
    \\%
    \ovalue = \osMagic{#1} \\
