...
\begin{document}
  ...
  \begin{mathpar}
    \relationRule{If Result (\ovalvariable)}{
      ...
      \gsAddEdge{\gnode_2 \gsBefore \gsLR{\gsifresult, \gtime + 1}}
    }
  \end{mathpar}
  ...
  \begin{mathpar}
    \relationRule{Return}{
      ...
      \gsAddEdge{\gnode_2 \gsBefore \gsLR{\gsadvance[_0], \gtime + 1}}
    }
  \end{mathpar}
  ...
  \begin{definition}[Lookup]
    Let \osFunc{Lookup}(\ggraph, \gnode, \lstack) be the function returning the least set \lresults \;such that the following rules hold for $\gnode_1 \gsBefore \gnode_0$ :
