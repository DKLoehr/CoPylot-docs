...
\begin{document}
  ...
  \begin{grammar}
    ...
    \grule[statements]{\pstmt}{
        ...
        \gor    \gtwhile \pexpr \gtcolon \psuite \gtelse \gtcolon \psuite \gline
        \gor    \gtif \pexpr \gtcolon \psuite \gtelse \gtcolon \psuite \gline
        \gor    \gtraise \pexpropt, \pexpropt, \pexpropt \gline
        \gor    \gttry \gtcolon \psuite \gtobrk \pexcepthandler; \ldots \gtcbrk \gtelse \gtcolon \psuite \gline
        \gor    \pexpr \gline
        \gor    \gtpass \gline
        \gor    \gtbreak \gline
...
Additionally, for all $e$, we will define $S_{(e)}$ and $\pvariable_{(e)}$ to be such
that $\meaningof{e} = \gsLR{S_{(e)}, \pvariable_{(e)}}$. We may also use this notation
for slices.
...
\begin{mathpar}
  \translationRule{Suite (statement list)}{
    ...
    \gtobrk \meaningof{\pstmt_1} \listConcat \ldots \listConcat \meaningof{\pstmt_n} \gtcbrk
  }{
  }
\end{mathpar}
...
$\ldots$
$\tsubsuite{e_n}$
$\pvariable$ = $\gtclass \gtopar \tsubvar{\pexpr_0}, \ldots, \tsubvar{\pexpr_n} \gtcpar$:
  $\meaningof{\psuite}$
\end{python}
\end{lrbox}
...
\begin{python}
$\tsubsuite{\pexpr}$
$\meaningof{\pexpr_1 = \tsubvar{\pexpr}}$
...
$\meaningof{\pexpr_n = \tsubvar{\pexpr}}$
\end{python}
\end{lrbox}
...
$\tsubsuite{\pexpr_1}$
$\tfreshvar$ = $s$
$\tsubvar{\pexpr_1}$.__setattr__($\tfreshvar$, $\tsubvar{\pexpr_2}$)
\end{python}
\end{lrbox}
...
$\tsubsuite{\pexpr_1}$
$\tsubsuite{\pslice}$
$\tsubvar{\pexpr_1}$.__setitem__($\tsubvar{\pslice}$, $\tsubvar{\pexpr_2}$)
\end{python}
\end{lrbox}
...
try:
  ...
  $\tfreshvar_0$ = $\tfreshvar_1$($\tsubvar{\pexpr}$)
...
if $\tfreshvar_0$ is $\star$NotImplemented and
  type($\pvariable$) is not type($\tsubvar{\pexpr}$):
  try:
    $\tfreshvar_1$ = $\tsubvar{\pexpr}$.ropFunc
  except $\star$AttributeError:
    $\tfreshvar_0$ = $\star$NotImplemented
  else:
...
\begin{mathpar}
  ...
  \mbox{where }
    ...
    \mathtt{ropFunc} = \osFunc{binop\_to\_rfunc}(\pbinop)
  }
\end{mathpar}
...
\begin{python}
$\tsubsuite{\pexpr_1}$
$\tfreshvar_1$ = $\tsubvar{\pexpr_1}$.$\pvariable$
$\tsubsuite{\pexpr_2}$
try:
  ...
  $\tfreshvar_0$ = $\tfreshvar_2$($\tsubvar{\pexpr_2}$)
...
if $\tfreshvar_0$ is $\star$NotImplemented and
  type($\tfreshvar_1$) is not type($\tsubvar{\pexpr_2}$):
  try:
    $\tfreshvar_2$ = $\tsubvar{\pexpr_2}$.ropFunc
  except $\star$AttributeError:
    $\tfreshvar_0$ = $\star$NotImplemented
  else:
...
if $\tfreshvar_0$ is $\star$NotImplemented:
  raise TypeError("Unsupported operands for binop")
    ...
    $\tsubvar{\pexpr_1}$.$\pvariable$ = $\tfreshvar_0$
\end{python}
\end{lrbox}
...
$\tsubsuite{\pexpr_1}$
$\tsubsuite{\pslice}$
$\tfreshvar_1$ = $\tsubvar{\pexpr_1}$[$\tsubvar{\pslice}$]
$\tsubsuite{\pexpr_2}$
$\tfreshvar$ = $\tsubvar{\pexpr_2}$
try:
  $\tfreshvar_2$ = $\tfreshvar_1$.iopFunc
except $\star$AttributeError:
  $\tfreshvar_0$ = $\star$NotImplemented
...
if $\tfreshvar_0$ is $\star$NotImplemented:
  raise TypeError("Unsupported operands for binop")
    ...
    $\tsubvar{\pexpr_1}$[$\tsubvar{\pslice}$] = $\tfreshvar_0$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{For loop}{
    \gtfor \pexpr_1 \gtin \pexpr_2 \gtcolon \psuite_1 \gtelse \gtcolon \psuite_2
  }{
...
\begin{python}
$\tsubsuite{\pexpr}$
$\llbracket\tfreshvar = \star$bool$(\tsubvar{\pexpr})\rrbracket$
while $\tfreshvar$:
  $\meaningof{\psuite_1}$
  $\tsubsuite{\pexpr}$
  $\llbracket\tfreshvar = \star$bool$(\tsubvar{\pexpr})\rrbracket$
...
\begin{mathpar}
  \translationRule{While loop}{
    \gtwhile \pexpr \gtcolon \psuite_1 \gtelse \gtcolon \psuite_2
  }{
    \usebox{\whileBox}
  }{
  }
...
\begin{mathpar}
  \translationRule{Try/Except statement}{
    \gttry \gtcolon \psuite_1 \gtobrk \pexcepthandler_1; \ldots; \pexcepthandler_n \gtcbrk \gtelse \gtcolon \psuite_2
  }{
    \usebox{\tryBox}
  }{
  }
...
We expect every expression to evaluate to a pair
$\meaningof{\pexpr} = \left<\tsubsuite{\pexpr}, \tsubvar{\pexpr}\right>$.
We will use the convention that $\left<\psuite, \meaningof{\pexpr}\right>$ is to be interpreted as
$\left<\psuite || \tsubsuite{\pexpr}, \pvariable_{(\pexpr)}\right>$.
...
\begin{mathpar}
  \translationRule{Boolean expression (And)}{
    ...
    \meaningof{\pexpr_2 \gtif \tsubvar{\pexpr_1} \gtelse \tsubvar{\pexpr_1}}\right>
  }{
  }
\end{mathpar}
...
\begin{mathpar}
  \translationRule{Boolean expression (Or)}{
    ...
    \left<\usebox{\andBox}\hspace{25pt},
    \meaningof{\tsubvar{\pexpr_1} \gtif \tsubvar{\pexpr_1} \gtelse \pexpr_2}\right>
  }{
  }
\end{mathpar}
...
$\tsubsuite{\pexpr_2}$
try:
  $\tfreshvar_3$ = $\tsubvar{\pexpr_1}$.opFunc
except $\star$AttributeError:
  $\tfreshvar_0$ = $\star$NotImplemented
else:
  $\tfreshvar_0$ = $\tfreshvar_3$($\tsubvar{\pexpr_2}$)

if $\tfreshvar_0$ is $\star$NotImplemented and
   type($\tsubvar{\pexpr_1}$) is not type($\tsubvar{\pexpr_2}$):
  try:
    $\tfreshvar_3$ = $\tsubvar{\pexpr_2}$.ropFunc
  except $\star$AttributeError:
    $\tfreshvar_0$ = $\star$NotImplemented
  else:
    $\tfreshvar_0$ = $\tfreshvar_3$($\tsubvar{\pexpr_1}$)
...
\begin{mathpar}
  \translationRule{Binary Operation}{
    ...
    \left<\left\llbracket\usebox{\binopBox}\right\rrbracket\hspace{0pt}, \tfreshvar_0 \right>
  }{
  \mbox{where }
  \mathtt{opFunc} = \osFunc{binop\_to\_func}(\pbinop) \\
  \mathtt{opFunc} = \osFunc{binop\_to\_rfunc}(\pbinop)
  }
\end{mathpar}
...
\begin{python}
$\tsubsuite{\pexpr}$
$\tfreshvar_1 = \star$bool
$\tfreshvar_2 = \tfreshvar_1(\tsubvar{\pexpr})$
$\tfreshvar_3 = \gtnot (\tfreshvar_2)$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Unary Operation (not)}{
    ...
    \left<\usebox{\notBox}\hspace{35pt}, \tfreshvar_3 \right>
  }{
  }
\end{mathpar}
...
\begin{mathpar}
  \translationRule{Unary Operation (+)}{
    ...
    \left<\usebox{\uplusBox}\hspace{20pt}, \meaningof{\tsubvar{\pexpr}.\mathtt{\_\_pos\_\_}()} \right>
  }{
  }
\end{mathpar}
...
\begin{mathpar}
  \translationRule{Unary Operation (-)}{
    ...
    \left<\usebox{\usubBox}\hspace{20pt}, \meaningof{\tsubvar{\pexpr}.\mathtt{\_\_neg\_\_}()} \right>
  }{
  }
\end{mathpar}
...
$\tsubsuite{\pexpr_2}$
$\tfreshvar_1 = \star$bool
$\tfreshvar_2$ = $\tfreshvar_1(\tsubvar{\pexpr_2}$)
if $\tfreshvar_2$:
  $\tsubsuite{\pexpr_1}$
  $\tfreshvar$ = $\tsubvar{\pexpr_1}$
else:
  $\tsubsuite{\pexpr_3}$
  $\tfreshvar$ = $\tsubvar{\pexpr_3}$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{If expression}{
    ...
    \left< \usebox{\ifExpBox}\hspace{50pt}, \tfreshvar \right>
  }{
  }
\end{mathpar}
...
...
$\tsubsuite{\pexpr_n}$
$\tfreshvar = \tsubvar{\pexpr}(\tsubvar{\pexpr_1}, \ldots, \tsubvar{\pexpr_n})$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Function call}{
    ...
    \left< \usebox{\callBox}\hspace{85pt},
    \tfreshvar \right>
  }{
  }
\end{mathpar}
...
\begin{python}
$\tsubsuite{\pexpr}$
$\tfreshvar = \tsubvar{\pexpr}.\pvariable$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Attribute access}{
    ...
    \left< \usebox{\attrBox}\hspace{45pt},
     \tfreshvar \right>
  }{
  }
\end{mathpar}
...
$\tsubsuite{\pexpr}$
$\tsubsuite{\pslice}$
$\tfreshvar_1 = \tsubvar{\pexpr}.\mathtt{\_\_getitem\_\_}$
$\tfreshvar_2 = \tfreshvar_1 ( \tsubvar{\pslice} )$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Subscript}{
    ...
    \left< \usebox{\listAccessBox}\hspace{90pt},
    \tfreshvar_2
    \right>
  }{
  }
...
...
$\tsubsuite{\pexpr_n}$
$\tfreshvar = \gtobrk \tsubvar{\pexpr_1}, \ldots, \tsubvar{\pexpr_n} \gtcbrk$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{List expression}{
    ...
    \left< \usebox{\listBox}\hspace{80pt},
    \tfreshvar \right>
  }{
  }
\end{mathpar}
...
...
$\tsubsuite{\pexpr_n}$
$\tfreshvar = \gtopar \tsubvar{\pexpr_1}, \ldots, \tsubvar{\pexpr_n} \gtcpar$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Tuple expression}{
    ...
    \left< \usebox{\tupleBox}\hspace{80pt},
    \tfreshvar \right>
  }{
  }
\end{mathpar}
...
\begin{mathpar}
  \translationRule{None expression}{
    ...
    \left< \tfreshvar \gteq \mathtt{\star None} , \tfreshvar \right>
  }{
  }
\end{mathpar}
...
\begin{mathpar}
  \translationRule{Literal expression}{
    ...
    \left< \tfreshvar = \pexpr , \tfreshvar \right>
  }{
    \mbox{where } \pexpr \mbox{ is a number, string, or boolean}
  }
\end{mathpar}

\begin{mathpar}
  \translationRule{Variable expression}{
    \pvariable
  }{
    \left< \gtobrk \gtcbrk, \pvariable \right>
  }{
  }
\end{mathpar}
...
$\tsubsuite{\pexpr_2}$
$\tsubsuite{\pexpr_3}$
$\meaningof{\tfreshvar = \star\mathtt{slice}(\tsubvar{\pexpr_1}, \tsubvar{\pexpr_2}, \tsubvar{\pexpr_3})}$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Slice (slice)}{
    ...
    \left< \usebox{\sliceBox}\hspace{130pt}, \tfreshvar \right>
  }{
    \mbox{where } \tsubsuite{\pexpr_i} \mbox{ is empty and } \tsubvar{\pexpr_i}
...
\begin{mathpar}
  \translationRule{Exception handler list (0 arg)}{
...
\begin{python}
$\tsubsuite{\pexpr}$
$\tfreshvar = \pvariable$.__class__
if $\tfreshvar$ is $\tsubvar{\pexpr}$:
  $\meaningof{\psuite}$
else:
  $\meaningof{\gtobrk \pexcepthandler_2; \ldots; \pexcepthandler_n \gtcbrk}_{\pvariable}$
\end{python}
...
\begin{mathpar}
  \translationRule{Exception handler list (1 arg)}{
...
\begin{mathpar}
  \translationRule{Exception handler list (2 arg)}{
...
\begin{definition}[Binary Operators]
  ...
  \begin{array}[t]{ c | c c c }
   \pbinop & \osFunc{Binop\_to\_func}(\pbinop) &
             \osFunc{Binop\_to\_ifunc}(\pbinop) &
             \osFunc{Binop\_to\_rfunc}(\pbinop) \\
   \hline
...
\begin{mathpar}
  \translationRule{While loop}{
    \gtwhile \pvariable \gtcolon \psuite_1 \gtelse \gtcolon \psuite_2
  }{
...
\begin{mathpar}
  \translationRule{Try/Except block}{
    \gttry \gtcolon \psuite_1 \gtexcept \pvariable \gtcolon \psuite_2 \gtelse \gtcolon \psuite_3
  }{
    \usebox{\lamiaTryBox}
  }{
  }
...
\begin{mathpar}
  \translationRule{Class Value}{
    \gtclass \pvariable(\pvariable_1, \ldots, \pvariable_n) \gtcolon \psuite
  }{
    \left<
    \usebox{\lamiaClassBox}\hspace{5pt},
    \tfreshmem_5
    \right>
  }{
    % No premises
...
\begin{definition}[Class creation]\ \\
