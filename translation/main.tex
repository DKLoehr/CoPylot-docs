...
\begin{python}
$\tsubsuite{\pexpr}$
$\tfreshvar = \gtnot (\star\gtbool(\tsubvar{\pexpr}))$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Unary Operation (not)}{
    ...
    \left<\usebox{\notBox}\hspace{100pt}, \tfreshvar \right>
  }{
  }
\end{mathpar}
...
$\tsubsuite{\pexpr}$
$\tsubsuite{\pslice}$
$\tfreshvar = \tsubvar{\pexpr}.\mathtt{\_\_getitem\_\_} ( \tsubvar{\pslice} )$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Subscript}{
    ...
    \left< \usebox{\listAccessBox}\hspace{105pt},
    \tfreshvar
    \right>
  }{
  }
...
TODO: Not sure if we need to bind None to a temp variable here, but I'm
doing it to be consistent
...
$\tsubsuite{\pexpr_2}$
$\tsubsuite{\pexpr_3}$
$\tfreshvar = \star\mathtt{slice}(\tsubvar{\pexpr_1}, \tsubvar{\pexpr_2}, \tsubvar{\pexpr_3})$
\end{python}
\end{lrbox}
...
\begin{mathpar}
  \translationRule{Slice (slice)}{
    ...
    \left< \usebox{\sliceBox}\hspace{120pt}, \tfreshvar \right>
  }{
...
\begin{python}
$\tsubsuite{\pexpr}$
if $\tfreshvar$.__class__ is $\tsubvar{\pexpr}$:
  $\meaningof{\psuite}$
else:
  $\meaningof{\gtobrk \pexcepthandler_2; \ldots; \pexcepthandler_n \gtcbrk}_{\pvariable}$
\end{python}
...
\begin{python}
$\tsubsuite{\pexpr}$
if $\pvariable$.__class__ is $\tsubvar{\pexpr}$:
