...
\end{python}
\end{lrbox}
TODO: Need to change use of Lookup to reflect the fact it returns a memvar. Sigh.
\begin{definition}[Get callable]\ \\
$\osFunc{get\_call}(\pvariable)$ = $\gtdef (\tfreshval) : \{ \psuite \}$, \\
where $\psuite = $
$\usebox{\GetCallBox}$
\end{definition}
