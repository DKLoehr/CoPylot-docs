...
$\meaningof{\psuite_1}$
$\meaningof{\psuite_2}$
$\pexpr_1'$.__setitem__($\pexpr_3'$, $\pexpr_2'$)
\end{python}
\end{lrbox}
...
TODO: This rule reflects the current code, but is wrong. It also needs to
check for \_\_radd\_\_ and maybe some more stuff, and should throw a
TypeError if it can't figure out how to do the addition. Also, the value
...
\begin{mathpar}
  \translationRule{Augmented Attribute Assignment}{
    ...
    \meaningof{\pexpr_1} = \gsLR{\psuite, \pexpr_1'} \\
    \mathtt{opFunc} = \osFunc{binop\_to\_func}(\pbinop) \\
    \mathtt{iopFunc} = \osFunc{binop\_to\_ifunc}(\pbinop)
  }
\end{mathpar}
...
$\tfreshvar_2$ = $\pexpr_3'$
try:
  $\tfreshvar_3$ = $\pexpr_1'$[$\pexpr_3'$].iopFunc
except AttributeError:
  $\tfreshvar_2$ = $\pexpr_1'$[$\pexpr_3'$].opFunc
$\pexpr_1'$[$\pexpr_3'$] = $\tfreshvar_2$($\pexpr_2$)
\end{python}
\end{lrbox}
